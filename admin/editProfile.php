...
$email=$_SESSION['email'];
    $sql="UPDATE `users` SET `username` = '$username', `address` = '$address' ,`mobile`='$phone' WHERE `users`.`email` = '$email'";
    $result=mysqli_query($connect,$sql);
    if($result){
      echo '<Script>
...
<section class="background-radial-gradient overflow-hidden">
  ...
  <div class="mb-3">
    ...
    <input type="text" value="<?php echo "$username";?>" name="username" class="form-control" id="name"  required>
  </div>
  <div class="mb-3">
    <label for="email" class="form-label">Email</label>
    <input type="email" value="<?php echo "$email";?>" name="email" class="form-control" id="email" aria-describedby="emailHelp" disabled>
    <div id="emailHelp" class="form-text">We'll never share your email with anyone else.</div>
  </div>
 
  <div class="mb-3">
    <label for="Phone" class="form-label">Phone</label>
    <input type="number" name="mobile" value="<?php echo "$phone";?>" class="form-control" id="phone" maxlength="10" length="10" required>
  </div>
  <div class="mb-3">
